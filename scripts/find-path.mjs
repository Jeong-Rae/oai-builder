...
const wormholePairs = map.wormholePairs ?? [
  {
    id: 1,
    positions: map.tiles.flatMap((row, y) =>
    ...
    entities: Object.fromEntries(
      map.objects.map((object) => [
        object.kind === "player" ? "player" : object.id,
        {
          ...
          controls: object.kind === "player" ? Object.keys(directions) : [],
        },
      ]),
    ),
...
function nextState(state, direction) {
  ...
  if (targetEntity) {
    if (targetEntity.kind === "anchor" && targetEntity.controls.length > 0) {
      ...
      next.entities[owner.id].controls = next.entities[owner.id].controls.filter(
        (item) => item !== direction,
      );
  ...
  if (tileAt(target) === "wormhole") {
    destination = wormholePairs
      .find((pair) =>
        pair.positions.some((position) => position.x === target.x && position.y === target.y),
      )
      ?.positions.find((position) => position.x !== target.x || position.y !== target.y);
  ...
  if (moving.kind === "normal") {
    if (fromTile === "plate")
      next.plateStates[`${owner.position.x},${owner.position.y}`] = "inactive";
    if (tileAt(destination) === "plate")
      next.plateStates[`${destination.x},${destination.y}`] = "active";
  }
