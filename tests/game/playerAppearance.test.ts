...
describe('플레이어 방향 스프라이트', () => {
  ...
  it('벽 또는 웜홀과 상호작용하면 기본 방향으로 돌아간다', () => {
    const wallGame = createInitialState({ boxCount: 0, tileOverrides: [{ position: { x: 0, y: 7 }, kind: 'wall' }] });
    const wormholeGame = createInitialState({
      boxCount: 0,
      tileOverrides: [{ position: { x: 0, y: 7 }, kind: 'wormhole' }, { position: { x: 2, y: 2 }, kind: 'wormhole' }],
    });

    expect(playerTextureForMove(wallGame, 'up', decide(wallGame, { type: 'player/move', direction: 'up' }))).toBe(playerTextureKeys.default);
    expect(playerTextureForMove(wormholeGame, 'up', decide(wormholeGame, { type: 'player/move', direction: 'up' }))).toBe(playerTextureKeys.default);
  });
});
