...
export class ChapterScene extends Phaser.Scene {
  ...
  init(data: { activeIndex?: number } = {}): void {
  ...
  private renderTitle(): void {
    const title = this.add
      .text(WIDTH / 2, 80, "CHAPTER SELECT", {
        color: "#ffffff",
    ...
    this.add
      .image(
        title.getLeftCenter().x - starGap - starWidth / 2,
        title.getCenter().y,
        "chapter-title-star",
      )
    ...
    this.add
      .image(
        title.getRightCenter().x + starGap + starWidth / 2,
        title.getCenter().y,
        "chapter-title-star",
      )
      .setDisplaySize(starWidth, starHeight)
  ...
  private createCard(index: number, x: number, scale: number, alpha: number): CarouselCard {
    ...
    view.setInteractive(
      new Phaser.Geom.Rectangle(-225, -300, 450, 600),
  ...
  private startChapter(chapterIndex: number, valid: boolean): void {
    ...
    this.scene.start("stage-select", { chapterIndex });
  }
  ...
  private renderChapter(chapter: ChapterDefinition): Phaser.GameObjects.Container {
    ...
    const points = chapter.constellation.map(
      ({ x, y }) => new Phaser.Math.Vector2(x - 225, y - 300),
    );
    ...
    points.forEach(({ x, y }) => {
      const glow = this.add
        .image(x, y, "chapter-constellation-star")
  ...
  private move(offset: -1 | 1): void {
    ...
    targets.forEach(([card, , scale]) =>
      card.view.setDepth(scale === 1 ? SIGN_CURRENT_DEPTH : scale === 0.55 ? 0 : SIGN_SIDE_DEPTH),
    );

    if (this.reducedMotion) {
      targets.forEach(([card, x, scale, alpha]) =>
        card.view.setPosition(x, CARD_Y).setScale(scale).setAlpha(alpha),
      );
      finish();
      return;
    }
